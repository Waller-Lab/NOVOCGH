...
if System.useGPU ==1;Masks = gpuArray(Images); KickMasks = gpuArray(KickImages);else;    Masks = Images; KickMasks = KickImages;    end;
...
f = figure(1); 
for i = 1:10
subplot(2,10,i);    imagesc(squeeze(Images(:,:,i))); axis off; axis image; caxis([0 2]);
subplot(2,10,10+i);    imagesc(squeeze(KickImages(:,:,i))); axis off; axis image;    caxis([0 2]); 
end
...
[ NOVOCGH.hologram,NOVOCGH.phase ] = function_NOVO_CGH_dualbinary( System, HStacks, Masks,KickMasks,Depths,NormOptions );
